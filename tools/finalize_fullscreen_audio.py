...
# 1) Name/Animal/Object native: remove old system-click audio path and use the
# shared real SFX engine so the global sound setting is respected.
...
if '_sound(' in s or 'SystemSoundType' in s or 'SystemSound.play' in s:
    raise SystemExit('Old system sound path remains in name game')
p.write_text(s, encoding='utf-8')

# 2) Four-player domino: fixed viewport (no horizontal scrollers), richer
# presentation, and shared real SFX for move/error/win.
...
if old not in s:
    raise SystemExit('Four-player domino scroll block not found')
...
if 'SingleChildScrollView(' in s or 'ListView(' in s:
    raise SystemExit('Scroller remains in four-player domino')
p.write_text(s, encoding='utf-8')

# 3) XO web client: true viewport, no page scroll, shared WebAudio context and
# distinct tap/move/win cues.
...
s = s.replace(
    'button{border:0;background:#6f2dbd;color:#fff;font-weight:900}',
    'button{border:0;background:linear-gradient(135deg,#0f766e,#6d28d9);color:#fff;font-weight:900}',
    1,
)
s = s.replace(
...
s = s.replace(
    "b.onclick=()=>{if(ws&&state.turnId===id&&!state.finished)ws.send(JSON.stringify({type:'move',index:i}));};",
    "b.onclick=()=>{if(ws&&state.turnId===id&&!state.finished){sfx('move');ws.send(JSON.stringify({type:'move',index:i}));}};",
    1,
)
s = s.replace(
...
s = s.replace(
    "function draw(){document.getElementById('status').textContent=state.message||'';",
    "function draw(){if(state.finished&&!prevFinished)sfx('win');prevFinished=!!state.finished;document.getElementById('status').textContent=state.message||'';",
    1,
)
p.write_text(s, encoding='utf-8')

# 4) Checkers web: fixed viewport and WebAudio. Board is sized by viewport, not page.
p = Path('lib/games/checkers/checkers_iphone_web.dart')
s = p.read_text(encoding='utf-8')
s = s.replace(
    '*{box-sizing:border-box}body{margin:0;padding:14px;background:#101820;color:#fff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}',
    '*{box-sizing:border-box}html,body{height:100%;overflow:hidden}body{margin:0;padding:8px;background:linear-gradient(145deg,#071a18,#172554);color:#fff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}',
    1,
)
s = s.replace(
    '.card{background:#192734;border-radius:20px;padding:14px;margin-bottom:12px}',
    '.card{background:#102a34;border:1px solid #ffffff16;border-radius:18px;padding:9px;margin-bottom:7px}',
    1,
)
s = s.replace(
    '.board{display:grid;grid-template-columns:repeat(8,1fr);aspect-ratio:1;border:5px solid #6b4f2a;border-radius:10px;overflow:hidden}',
    '.board{display:grid;grid-template-columns:repeat(8,1fr);width:min(82vh,96vw,620px);aspect-ratio:1;border:5px solid #ffb703;border-radius:14px;overflow:hidden;margin:auto;box-shadow:0 12px 30px #0008}',
    1,
)
...
# 5) Name game web: no horizontal/vertical page scrollers; reuse a single unlocked
# AudioContext. Results table scales to the viewport instead of scrolling.
p = Path('lib/games/name_animal_object/iphone_web_bridge.dart')
s = p.read_text(encoding='utf-8')
s = s.replace(
    ':root{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#20152f;background:#f6f3ff}*{box-sizing:border-box}body{margin:0;padding:max(18px,env(safe-area-inset-top)) 14px 30px;min-height:100vh}',
    ':root{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#0f172a;background:#f4f7fb}*{box-sizing:border-box}html,body{height:100%;overflow:hidden}body{margin:0;padding:max(8px,env(safe-area-inset-top)) 8px max(8px,env(safe-area-inset-bottom));height:100dvh}',
    1,
)
s = s.replace(
    '.card{background:#fff;border-radius:22px;padding:17px;margin:11px 0;box-shadow:0 10px 28px #3c17651a}.hero{background:linear-gradient(135deg,#6f2dbd,#148f77);color:#fff;text-align:center}.letter{font-size:72px;font-weight:900}',
    '.card{background:#fff;border-radius:18px;padding:10px;margin:6px 0;box-shadow:0 8px 22px #0f172a18}.hero{background:linear-gradient(135deg,#6d28d9,#0f766e);color:#fff;text-align:center}.letter{font-size:clamp(42px,9vh,68px);font-weight:900}',
    1,
)
s = s.replace(
    '.table-wrap{overflow:auto;border:1px solid #ddd3ef;border-radius:16px}table{border-collapse:collapse;min-width:720px;width:100%;background:#fff}',
    '.table-wrap{overflow:hidden;border:1px solid #ddd3ef;border-radius:16px;max-height:60vh}table{border-collapse:collapse;width:100%;background:#fff;font-size:clamp(7px,2vw,12px);table-layout:fixed}',
    1,
)
...
# Make active sections occupy the viewport without body scrolling.
...
# Final native contract: no ListView or SingleChildScrollView in any playable screen.
active = [
...
for f in active:
    ...
    for forbidden in ('SingleChildScrollView(', 'ListView('):
        if forbidden in data:
            raise SystemExit(f'{forbidden} remains in {f}')
print('final fullscreen/audio pass applied')
