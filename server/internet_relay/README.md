...
يتضمن جذر المستودع ملف `render.yaml` لتشغيل الخدمة كـ Docker Web Service في
منطقة Frankfurt، مع فحص صحة `/health`. بعد إنشاء Blueprint من المستودع، انسخ
عنوان الخدمة وأضف إليه `/ws` ثم خزّنه في متغير GitHub باسم
`INTERNET_RELAY_URL` قبل بناء APK النهائي.
