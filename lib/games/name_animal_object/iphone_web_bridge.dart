...
<section id="wait" class="card hidden"><h2>تم الاتصال ✅</h2><p class="muted">بانتظار بدء اللعبة...</p></section>
...
function join(){audio();beep(520,.05,.035);const name=document.getElementById('name').value.trim();if(!name)return;ws=new WebSocket(`ws://${location.host}/ws`);ws.onopen=()=>ws.send(JSON.stringify({type:'join',name}));ws.onmessage=e=>handle(JSON.parse(e.data));ws.onclose=()=>{alert('انقطع الاتصال بصاحب الغرفة');show('join')}}
