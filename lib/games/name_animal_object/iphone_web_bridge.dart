...
:root{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#20152f;background:#f6f3ff}*{box-sizing:border-box}body{margin:0;padding:max(18px,env(safe-area-inset-top)) 14px 30px;min-height:100vh}.card{background:#fff;border-radius:22px;padding:17px;margin:11px 0;box-shadow:0 10px 28px #3c17651a}.hero{background:linear-gradient(135deg,#6f2dbd,#148f77);color:#fff;text-align:center}.letter{font-size:72px;font-weight:900}.row{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.pill{padding:11px;border-radius:14px;background:#eee8ff;text-align:center;font-weight:800}input{width:100%;font-size:18px;padding:14px;border:1px solid #ddd3ef;border-radius:14px;margin:5px 0 9px}button{width:100%;border:0;border-radius:15px;padding:14px;font-size:17px;font-weight:900;background:#6f2dbd;color:white}.hidden{display:none}.muted{color:#746b7e;text-align:center}.table-wrap{overflow:auto;border:1px solid #ddd3ef;border-radius:16px}table{border-collapse:collapse;min-width:720px;width:100%;background:#fff}th,td{border:1px solid #ddd3ef;padding:11px;text-align:center;white-space:nowrap}th{background:#eee8ff;position:sticky;top:0}.score{font-weight:900;color:#148f77}.danger{background:#c62828}.vote{display:flex;gap:8px}.vote button{width:auto;flex:1}.ok{background:#148f77}
</style></head><body>
...
<script>
const cats=['اسم','حيوان','جماد','نبات','بلاد'];let ws,playerId,currentRound=0,timer,currentProposal='';
const beep=(f=650,d=.12)=>{try{const a=new(window.AudioContext||window.webkitAudioContext)(),o=a.createOscillator(),g=a.createGain();o.frequency.value=f;o.connect(g);g.connect(a.destination);g.gain.value=.08;o.start();o.stop(a.currentTime+d)}catch(e){}};
