...
class _DominoFourPlayerScreenState extends State<DominoFourPlayerScreen> {
  ...
  void _newGame() {
    ...
    board.clear(); left=null; right=null; turns.reset(); message='دور اللاعب 1';
    if (mounted) setState(() {});
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: Column(children:[
      ...
      Wrap(spacing:8,children:List.generate(4,(i)=>Chip(avatar:CircleAvatar(child:Text('${i+1}')),label:Text('${hands[i].length} قطع'),backgroundColor:i==turns.currentPlayer?Colors.amber:null))),
      Padding(padding:const EdgeInsets.all(10),child:Text(message,style:Theme.of(context).textTheme.titleMedium)),
      ...
      const Divider(),
      Text('قطع اللاعب ${turns.currentPlayer+1}'),
      SizedBox(height:96,child:ListView(scrollDirection:Axis.horizontal,padding:const EdgeInsets.all(8),children:hands[turns.currentPlayer].map((t)=>_tile(t,true)).toList())),
      Padding(padding:const EdgeInsets.all(12),child:Row(children:[Expanded(child:OutlinedButton.icon(onPressed:_pass,icon:const Icon(Icons.skip_next),label:const Text('تمرير الدور'))),const SizedBox(width:8),Expanded(child:FilledButton.icon(onPressed:_newGame,icon:const Icon(Icons.restart_alt),label:const Text('لعبة جديدة')))]))
    ])),
  );
